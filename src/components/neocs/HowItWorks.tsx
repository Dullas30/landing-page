...
function GrowthChain({ progress }: { progress: import("motion/react").MotionValue<number> }) {
  ...
  return (
    <svg viewBox="0 0 300 940" fill="none" className="h-full w-full">
      ...
      />
      {nodes.map((pt, i) => (
        <GrowthNode key={i} progress={progress} index={i} pt={pt} />
      ))}
    </svg>
  );
}
...
function GrowthNode({
  ...
  const nodeProgress = useTransform(
    progress,
    [Math.max(0, (index - 0.4) / 5.5), (index + 0.4) / 5.5],
    [0, 1],
  );
  ...
  return (
    <motion.g style={{ scale, opacity }}>
      ...
      <circle
        ...
        cy={pt.y}
        r="9"
        fill={index === nodes.length - 1 ? "var(--color-harvest)" : "var(--color-growth)"}
      />
      <text
        x={pt.x}
        y={pt.y + 44}
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <Reveal delay={2}>
          <p className="mt-6 max-w-2xl text-base leading-relaxed text-muted-foreground">
            Watch the same loop that repeats at national scale, from one member's first contribution
            to prosperity shared across communities.
          </p>
        </Reveal>

        <div
          ref={ref}
          className="relative mt-16 grid gap-10 sm:mt-20 lg:grid-cols-[0.85fr_1.15fr] lg:gap-16"
        >
          ...
          <ol className="space-y-16 sm:space-y-20 lg:py-10">
            {stages.map((s, i) => (
              <Reveal as="li" key={s.n} delay={0}>
                <span className="font-display text-sm font-bold tracking-widest text-growth">
                  {s.n}
                </span>
                ...
                {i === stages.length - 1 && (
                  <p className="mt-6 max-w-md rounded-2xl border border-harvest/30 bg-harvest-soft/25 p-5 text-sm leading-relaxed text-foreground">
                    Every cycle strengthens the next: more members, more assets, more revenue,
                    flowing back to the people who put the capital to work.
                  </p>
                )}
              </Reveal>
            ))}
